...
static inline int parse_ipv4(void *data, u64 nh_off, void *data_end) {
    ...
    if(iph->protocol==IPPROTO_TCP && subnet == BLOCK_SUBNET) {
        ...
        unsigned int tcplen = (unsigned char)data_end - ((unsigned char)iph + nh_off);
        ...
        u16 check = tcph->check;
        u16 check_calc = tcp_v4_check(tcplen, iph->saddr, iph->daddr, csum_partial((char *)tcph, tcplen, 0));
        bpf_trace_printk("check: 0x%x check_calc: 0x%x\n", check, check_calc);
